...
class NucleiTemplates:
    ...
    def WrapperStart(self, silent=False):
        if not silent:
            print("-" * 75)
            print(f"{ConsoleColors.BLUE}[+] Searching in Nuclei Templates{ConsoleColors.ENDC}")
            print("-" * 75)
    ...
    def Output(self, data, limit, silent=False):
        for item in data:
            ...
            wrap = {
                ...
                "command" : f"nuclei -t ~/nuclei-templates/{path} -u [TARGET]",
                "searchBy": "code",
        ...
        try:
            ...
            if not self.api_key:
                 if not silent:
                    ...
                    print(f"{ConsoleColors.WARNING}[!] Please add your GitHub token to the config file to enable Nuclei Templates search.{ConsoleColors.ENDC}")
            ...
            while True:
                ...
                params = {
                    "q": f"{keyword} repo:projectdiscovery/nuclei-templates",
                    "page": page,
                    "per_page": 100
                }
            ...
            print(f"{ConsoleColors.FAIL}[-] Error in Nuclei Templates source [search]: {str(e)}{ConsoleColors.ENDC}")
    ...
    def wrapperEnd(self, silent=False):
        ...
        try:
            ...
            print(f"{ConsoleColors.FAIL}[-] Error in Nuclei Templates source [wrapperEnd]: {str(e)}{ConsoleColors.ENDC}")